...
import { Card, Badge, stateBadgeColor, Spinner, Button, Alert } from "@/components/ui";
...
export default function OnuDetailPage() {
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="mb-1.5 flex items-center gap-2 text-xs text-slate-500">
            <button onClick={() => router.push("/onus")} className="rounded-md border border-slate-300 bg-white px-2 py-1 hover:bg-slate-50">
              ← Kthehu
            </button>
          ...
          <div className="mt-0.5 text-xs text-slate-500">
            ...
            {epon && <> · <Badge color="gray">EPON</Badge></>}
            {connectionKind === "bridge" && <> · <Badge color="gray">Bridge</Badge></>}
            {connectionKind === "route" && <> · <Badge color="gray">Route</Badge></>}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" onClick={doRefresh} disabled={refreshing || epon} title={epon ? "Rifreskimi CLI nuk është ende i implementuar për EPON" : undefined}>
            {refreshing ? <Spinner /> : "🔄"} Rifresko nga OLT
          </Button>
          <Button variant="success" onClick={() => setPppoeOpen(true)} disabled={epon} title={epon ? "PPPoE nuk është i mbështetur për EPON ende" : undefined}>
            🔐 Ndrysho PPPoE
          </Button>
          ...
          <Button variant="danger" onClick={doReboot} disabled={rebooting || !wifi?.deviceId} title={!wifi?.deviceId ? "Nuk ka TR-069 për këtë ONU" : undefined}>
            {rebooting ? <Spinner /> : "⏻"} Riniso ONU
          </Button>
        </div>
      </div>

      {rebootMsg && <Alert kind={rebootMsg.kind}>{rebootMsg.text}</Alert>}

      <div className="grid gap-4 lg:grid-cols-2">
        <Card title={<>📋 Informacioni ONU <Badge color={stateBadgeColor(onu.state)}>● {onu.state === "working" ? "Online" : "Offline"}</Badge></>}>
          <div className="px-4">
            ...
            <InfoRow label="Tipi ONU" value={<Badge color="blue">{onu.type || "N/A"}</Badge>} />
            ...
            <InfoRow label="VLAN" value={<Badge color="blue">{onu.vlan || "N/A"}</Badge>} />
          </div>
        </Card>

        <div className="flex flex-col gap-3.5">
          <Card title={<>📶 Sinjali Optik</>}>
            <div className="p-4">
              {hasSignal ? (
                <>
          ...
          <Card title={<>🔐 WAN / PPPoE</>} action={!epon ? <Button variant="success" className="px-2 py-1 text-[11px]" onClick={() => setPppoeOpen(true)}>✏ Ndrysho</Button> : undefined}>
            <div className="px-4">
      ...
      {live?.history && (
        <Card title={<>📅 Historia e Lidhjes <span className="text-[11px] font-normal text-slate-400">{live.history.length} ngjarje</span></>} className="mt-4">
                ...
                {live.history.map((h, i) => (
                  <tr key={i} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-3.5 py-2">
                      <Badge color={h.cause === "LOS" ? "red" : h.cause === "-" ? "green" : "amber"}>{h.cause}</Badge>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Card title={<>📶 WiFi (TR-069)</>} className="mt-4" action={wifi ? <Button variant="primary" className="px-2 py-1 text-[11px]" onClick={() => setWifiOpen(true)}>✏ Modifiko WiFi</Button> : undefined}>
        <div className="p-4">
          {!onu.serial ? (
...
function WifiBand({ label, band }: { label: string; band?: { ssid: string; password: string; enabled: boolean } }) {
  return (
    <div className="flex-1 rounded-lg border border-slate-200 p-3">
      <div className="mb-2 text-[10px] font-bold uppercase tracking-wide text-slate-500">📡 {label}</div>
      ...
      <InfoRowSm label="Status" value={<Badge color={band?.enabled ? "green" : "red"}>{band?.enabled ? "Aktiv" : "Off"}</Badge>} />
    </div>
  );
}

function InfoRowSm({ label, value }: { label: string; value: React.ReactNode }) {
  return (

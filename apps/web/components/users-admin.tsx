...
export default function UsersPage() {
  ...
  async function sendInvite() {
    ...
    setMsg(null);
    try {
      const r = await api.inviteUser({ email: inviteEmail.trim() });
      setMsg({
        kind: "ok",
        text: r.warning
          ? r.warning + (r.inviteUrl ? ` · ${r.inviteUrl}` : "")
          : `Ftesa u dërgua${r.inviteUrl ? ` · ${r.inviteUrl}` : ""}`,
      });
      setInviteEmail("");
      await load();
    } catch (e) {
  ...
  return (
    <div>
      ...
      <Card className="mb-4 p-4">
        <div className="mb-2 text-sm font-semibold">Fto me email (invite-only)</div>
        <div className="flex flex-wrap gap-2">
          <Input
            className="max-w-xs"
            type="email"
            placeholder="koleg@kompania.al"
            value={inviteEmail}
            onChange={(e) => setInviteEmail(e.target.value)}
          />
          <Button onClick={sendInvite} disabled={busy || !/\S+@\S+\.\S+/.test(inviteEmail)}>
            Dërgo ftesë
          </Button>
        </div>
        <p className="mt-1 text-[11px] text-muted-foreground">Kërkon SMTP (Admin → Integrime → Email) dhe APP_BASE_URL.</p>
      </Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
                  ...
                  <tr key={u.id} className="border-b border-border/50 last:border-0">
                    ...
                    <td className="px-4 py-2">
                      <span
                        ...
                        title={u.emailVerifiedAt ? "Email i verifikuar" : "Email i paverifikuar"}
                      >
                        {u.status ?? "active"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2">
                      <div className="flex justify-end gap-1.5">
                        {u.status === "pending" && (
                          <Button
                            variant="secondary"
                            className="px-2 py-1 text-[11px] text-emerald-700"
                            onClick={async () => {
                              try {
                                await api.updateUser(u.id, { status: "active" });
                                setMsg({ kind: "ok", text: `${u.email} u miratua` });
                                await load();
                              } catch (e) {
                                setMsg({ kind: "err", text: e instanceof ApiError ? e.message : "Gabim" });
                              }
                            }}
                            title="Mirato llogarinë (active)"
                          >
                            <Check className="h-3.5 w-3.5" /> Mirato
                          </Button>
                        )}
              ...
              {users.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-6 text-center text-xs text-muted-foreground"><ShieldCheck className="mx-auto mb-1 h-5 w-5" />Asnjë përdorues</td></tr>
              )}
            </tbody>
          </table>
        </div>
